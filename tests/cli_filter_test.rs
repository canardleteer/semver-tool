...
fn cli_filter_test_boring_cases() {
    ...
    let mut cmd = Command::cargo_bin(env!("CARGO_PKG_NAME")).unwrap();
    let assert = cmd.arg(TEST_PKG_NAME).arg(">1").arg("x.y.z").assert();
    assert
        .append_context(TEST_PKG_NAME, "1 bad semver arg")
        .failure();

    let mut cmd = Command::cargo_bin(env!("CARGO_PKG_NAME")).unwrap();
    let assert = cmd.arg(TEST_PKG_NAME).arg("2.0.0").arg(">1").assert();
    assert
...
fn cli_filter_test_basic_cases() {
    let mut cmd = Command::cargo_bin(env!("CARGO_PKG_NAME")).unwrap();
    let assert = cmd.arg(TEST_PKG_NAME).arg(">1").arg("2.0.0").assert();
